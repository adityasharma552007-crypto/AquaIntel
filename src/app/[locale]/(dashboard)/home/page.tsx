...
import CodTrendChart from "@/components/CodTrendChart"
...
export default async function HomePage({ params: { locale } }: { params: { locale: string } }) {
  ...
  // Fetch recent scans
  const { data: waterScans } = await supabase
    .from('water_data')
    .select('*')
    .order('created_at', { ascending: false })
    .limit(3)

  const scans = waterScans || []
  ...
  const stats = [
    { label: t('totalScans'), value: profile?.total_scans || 0, icon: Activity, color: "text-blue-400" },
    { label: t('safeWater'), value: profile?.safe_scans || 0, icon: Shield, color: "text-[#60A5FA]" },
  ]
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <header className="p-6 pb-2 pt-6 flex items-center justify-between">
        <div>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">{t('welcomeBack')}</p>
        ...
        <Link href="/chat" className="flex items-center gap-2 bg-gradient-to-br from-blue-50 to-indigo-50 px-4 py-2 rounded-2xl shadow-sm border border-blue-100 hover:shadow-md hover:scale-105 transition-all group">
          ...
          <div className="flex flex-col">
            <span className="text-[9px] font-black uppercase tracking-widest text-[#60A5FA]/60 leading-none mb-0.5">AquaIntel</span>
            <span className="text-xs font-black uppercase tracking-wider text-[#60A5FA] leading-none">{t('askAi')}</span>
          </div>
        </Link>
      </header>
      ...
      <main className="flex-1 p-6 pt-0 space-y-6">
               ...
               <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[11px] font-black text-red-600 uppercase tracking-tight leading-tight">
                      {t('flaggedVendors', { count: flaggedCount })}
                    </p>
                  </div>
               </div>
                 ...
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       <h3 className="text-white font-black uppercase tracking-tighter text-lg leading-tight flex items-center gap-2">
                          {t('aiInsights')} <span className="bg-[#60A5FA] text-white text-[8px] px-1.5 py-0.5 rounded uppercase tracking-widest leading-none">{t('newBadge')}</span>
                       </h3>
                       <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-0.5">{t('cityRisk')}</p>
                    </div>
                 </div>
        ...
        <Card className="bg-[#60A5FA] border-none shadow-xl shadow-blue-100/50 rounded-3xl overflow-hidden relative">
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4">
              ...
              <Badge variant="outline" className="text-white border-white/20 bg-white/5 backdrop-blur-md">{t('aiActive')}</Badge>
            </div>
        ...
        {/* COD Trend Line Chart */}
        <div className="space-y-4">
          <CodTrendChart scans={scans as any} />
        </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-end">
            ...
            <Link href="/history" className="text-[10px] font-bold text-[#60A5FA] uppercase underline underline-offset-4">{t('viewAll')}</Link>
          </div>
          ...
          <div className="space-y-3">
            {scans && scans.length > 0 ? (
                    ...
                    <CardContent className="p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-slate-800 truncate">
                          {scan.location_name || t('homeSample')}
                        </p>
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-[9px] font-black uppercase text-slate-400 tracking-tighter">{t('safety')}</span>
                      </div>
                    </CardContent>
                  </Card>
                </Link>
              ...
              <div className="py-12 text-center bg-white rounded-3xl border border-dashed border-slate-200">
                <p className="text-slate-400 text-sm font-bold uppercase tracking-widest">{t('noScansYet')}</p>
                <p className="text-slate-300 text-xs mt-1">{t('startFirstScan')}</p>
              </div>
            )}
          </div>
        </div>
