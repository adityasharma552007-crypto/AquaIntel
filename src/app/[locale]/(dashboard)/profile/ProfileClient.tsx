    ...
    <div className="min-h-screen bg-[#F7F9F8] pb-32">
              ...
              reports.map(report => (
                <Card key={report.id} className="rounded-[24px] border-none shadow-sm p-4 flex items-center justify-between bg-white">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-bold text-[15px] text-[#60A5FA]">{report.complaint_ref || `AQ-2026-${report.id.slice(0,5)}`}</p>
                      <p className="text-[12px] text-slate-500 font-medium">
                        {report.water_data?.location_name ?? 'Unknown location'}
                      </p>
            ...
            <div className="flex flex-col pt-2">
              ...
              <div onClick={() => setSheetOpen('about')} className="flex items-center justify-between px-6 py-4 border-b border-slate-50 cursor-pointer hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-bold text-[15px] text-slate-900">About AquaIntel</span>
                </div>
                <ChevronRight size={18} className="text-slate-300" />
              </div>
              <div onClick={() => setSheetOpen('fssai')} className="flex items-center justify-between px-6 py-4 border-b border-slate-50 cursor-pointer hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-bold text-[15px] text-slate-900">Water Safety Standards</span>
                </div>
                <ChevronRight size={18} className="text-slate-300" />
              </div>
      ...
      <BottomSheet isOpen={sheetOpen === 'notifications'} onClose={() => setSheetOpen(null)} title="Notifications">
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <span className="font-medium text-slate-900">Authority report updates</span>
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-sm text-slate-500 leading-relaxed">
              AquaIntel stores your scan history securely in Supabase. Your data is never shared with third parties.
            </p>
          </div>
      ...
      <BottomSheet isOpen={sheetOpen === 'about'} onClose={() => setSheetOpen(null)} title="About AquaIntel">
        <div className="flex flex-col items-center text-center space-y-4 pt-4">
          <div className="w-20 h-20 bg-[#60A5FA] rounded-full flex items-center justify-center shadow-md">
            <Shield className="text-white" size={40} />
          </div>
          <div>
            <h3 className="text-xl font-black text-[#60A5FA]">AquaIntel</h3>
          ...
          <p className="text-[15px] text-slate-600 leading-relaxed px-4 py-2">
            AquaIntel is a contactless water adulteration detection system using NIR spectral analysis and AI.
          </p>
          ...
          <p className="text-sm font-bold text-[#60A5FA] pt-4 leading-relaxed">
            Protecting families from adulterated water — one scan at a time. 💧
          </p>
        </div>
      </BottomSheet>

      <BottomSheet isOpen={sheetOpen === 'fssai'} onClose={() => setSheetOpen(null)} title="EPA Water Safety Standards 📋">
        <div className="space-y-4">
          <table className="w-full text-left text-[13px]">
            <thead>
              <tr className="border-b border-slate-200 text-slate-500">
                <th className="py-3 font-bold">Contaminant</th>
                <th className="py-3 font-bold">Safe Limit</th>
                <th className="py-3 font-bold">Risk Level</th>
              </tr>
            </thead>
            <tbody className="text-slate-800">
              <tr className="border-b border-slate-50"><td className="py-3">Lead</td><td>&lt; 0.015 mg/L</td><td className="text-red-500 font-bold">Unsafe</td></tr>
              <tr className="border-b border-slate-50"><td className="py-3">Coliform Bacteria</td><td>Zero</td><td className="text-red-600 font-bold">Hazardous</td></tr>
              <tr className="border-b border-slate-50"><td className="py-3">Fluoride</td><td>&lt; 4.0 mg/L</td><td className="text-red-600 font-bold">Hazardous</td></tr>
              <tr className="border-b border-slate-50"><td className="py-3">Arsenic</td><td>&lt; 0.010 mg/L</td><td className="text-slate-600 font-bold">Unsafe</td></tr>
              <tr className="border-b border-slate-50"><td className="py-3">Chlorine</td><td>&lt; 4.0 mg/L</td><td className="text-amber-500 font-bold">Substandard</td></tr>
              <tr><td className="py-3">Nitrates</td><td>&lt; 10 mg/L</td><td className="text-amber-600 font-bold">Unsafe</td></tr>
            </tbody>
          </table>
          <p className="text-[11px] text-slate-400 font-medium pt-4 text-center border-t border-slate-100">
            EPA National Primary Drinking Water Regulations<br/>For reporting contact your local authority
          </p>
        </div>
      </BottomSheet>
    </div>
