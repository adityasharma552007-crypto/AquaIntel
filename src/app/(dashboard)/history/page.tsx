...
export default async function HistoryPage() {
  ...
  // Fetch all scans from new water_data table
  ...
  const trendData = last30Days.map((day: Date) => {
    ...
    const avgScore = dayScans.length > 0 
      ? dayScans.reduce((acc, s) => acc + ((s.quality || 0) * 100), 0) / dayScans.length 
      : null
        ...
        <div className="space-y-4">
           {scans && scans.length > 0 ? (
                    ...
                    <Card className="rounded-2xl border-none shadow-sm overflow-hidden hover:bg-slate-50 transition-colors">
                      <CardContent className="p-4 flex items-center gap-4">
                         <div className={cn(
                           ...
                           {scan.quality >= 0.8 ? <Shield size={24} /> : <AlertTriangle size={24} />}
                         </div>
                         <div className="flex-1 min-w-0">
                          <p className="font-bold text-slate-800 truncate">
                             {scan.location_name || 'Home Sample'}
                          </p>
                         ...
                         <div className="text-right">
                          ...
                          <Badge variant="outline" className={cn(
                               ...
                               scan.quality >= 0.8 ? "border-[#60A5FA] text-[#60A5FA]" : "border-red-500 text-red-500"
                             )}>
                               {scan.status || (scan.quality >= 0.8 ? 'Pure' : 'Contaminated')}
                             </Badge>
                         </div>
                      </CardContent>
                    </Card>
             ...
             <div className="py-20 text-center flex flex-col items-center">
                ...
                <p className="text-xs text-slate-300 mt-1">Your water purity history will appear here.</p>
             </div>
           )}
        </div>
      </main>
