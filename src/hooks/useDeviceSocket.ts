 ...
 *   ESP → App  { type: "reading",  progress, soap, lead, arsenic, bacteria?, temp? }
 *   ESP → App  { type: "result",   soap, lead, arsenic, bacteria, safetyScore, duration }
 *   ESP → App  { type: "pong" }
  ...
  const handleFrame = useCallback((raw: string) => {
    ...
    if (type === 'reading') {
      setLiveReading({
        ...
        arsenic:    Number(frame.arsenic    ?? 0),
        bacteria: Number(frame.bacteria ?? 0),
