...
// ─── Quality thresholds ───────────────────────────────────────────────────────
export const getStatusFromQuality = (quality: number): 'Pure' | 'Contaminated' | 'Uncertain' => {
  if (quality >= 0.8) return 'Pure';
  if (quality < 0.4) return 'Contaminated';
  return 'Uncertain';
};

/** @deprecated Use getStatusFromQuality — kept for backward compat */
export const getDerivativeStatus = (quality: number): string => {
  if (quality >= 0.8) return 'Pure Water';
  if (quality < 0.4) return 'Contaminated Water';
  return 'Uncertain';
};
...
export function useRealtimeScans() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('water_data_as7343_channel')
      .on(
        'postgres_changes',
      ...
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'water_data' },
        (payload) => {
