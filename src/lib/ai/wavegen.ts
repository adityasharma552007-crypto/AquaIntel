 ...
 * by the actual channel readings from water_data.
 */

// ─── Pure water spectral baseline (AS7343 14-channel) ────────────────────────
...
const SPIKES: Record<Profile, Partial<Record<number, number>>> = {
  pure: {},
  adulterated: {
    3:  +0.18,  // F4 ~515nm  — lead spike
    4:  +0.20,  // F5 ~555nm  — lead spike
    8:  +0.12,  // NIR        — water dilution elevation
  },
  hazardous: {
    2:  +0.15,  // F3 ~480nm  — bacteria zone
    3:  +0.25,  // F4 ~515nm  — bacteria spike
    4:  +0.22,  // F5 ~555nm  — bacteria spike
    8:  +0.35,  // NIR        — fluoride / heavy adulterant
  },
}
