...
import { reportWaterSource } from '@/lib/actions/reports'
...
export default function ReportButton({ scanId, isHazard, isReported }: ReportButtonProps) {
  ...
  const handleReport = async () => {
    startTransition(async () => {
       try {
         const res = await reportWaterSource(scanId)
         if (res.success) {
           setReported(true)
           toast.success("Source reported to Water Authority successfully.")
         }
       } catch (err: any) {
         toast.error(err.message || "Failed to report Source.")
       }
    })
  }
  ...
  if (reported) {
    return (
      <div className="flex items-center gap-2 p-4 bg-red-50 text-red-600 rounded-3xl border border-red-100">
         ...
         <div>
            <p className="font-black uppercase text-xs tracking-tighter">Reported to Authority</p>
