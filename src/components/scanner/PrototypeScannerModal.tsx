...
const RULES: Record<string, {
  ...
  lead: { name: 'Lead', channels: [4, 5], threshold: 0.12, limit: 0.07, unit: '%', maxConc: 0.5, type: 'max' },
  bacteria: { name: 'Bacteria', channels: [3, 4, 5], threshold: 0.20, limit: 0.0, unit: '%', maxConc: 1.0, type: 'zero_tolerance' },
  arsenic: { name: 'Arsenic', channels: [8, 9], threshold: 0.15, limit: 0.0, unit: '%', maxConc: 2.0, type: 'zero_tolerance' },
  fluoride: { name: 'Fluoride', channels: [16], threshold: 0.30, limit: 0.0, unit: '%', maxConc: 0.05, type: 'zero_tolerance' },
...
function nirToWavelengths(nirReading: number): number[] {
  ...
  // 1–40 → heavily contaminated (bacteria + fluoride spikes)
  // 41–79 → borderline (lead + water spikes)
  // 80–100 → pure / safe water (no spikes)
  const spikes: Record<number, number> = {}

  if (nirReading <= 40) {
    // Hazardous: bacteria + fluoride
    spikes[3] = +0.18; spikes[4] = +0.28; spikes[5] = +0.25; spikes[16] = +0.52
  } else if (nirReading <= 79) {
    // Borderline / Uncertain
    spikes[4] = +0.22; spikes[5] = +0.20; spikes[13] = +0.09
  }
  // 80–100: pure → no spikes
...
function detectAdulterant(key: string, rule: typeof RULES[string], readings: number[]) {
  ...
  } else if (key === 'bacteria' || key === 'fluoride') {
...
function getTier(score: number): 'safe' | 'warning' | 'danger' | 'hazard' {
  return score >= 80 ? 'safe' : score >= 40 ? 'warning' : score >= 20 ? 'danger' : 'hazard'
}
...
function getRecommendation(tier: string, adulterants: ReturnType<typeof detectAdulterant>[]) {
  ...
  switch (tier) {
    case 'safe': return 'This water is safe to consume. No significant adulterants detected.'
    case 'warning': return 'This water has minor quality issues. Not immediately dangerous but below FSSAI standards.'
    case 'danger': return 'This water is adulterated above safe limits. Do not give to children. Report this vendor.'
...
export function PrototypeScannerModal({ isOpen, onClose }: PrototypeScannerModalProps) {
  ...
  const stageLabel: Partial<Record<ScanStage, string>> = {
    connecting: 'Connecting to AquaIntel-ESP32-001...',
          ...
          {scanStage === 'input' && (
            <motion.div key="input" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5 py-2">
              <div className="text-center">
                <p className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-1">Device Ready — AquaIntel-ESP32-001</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs">
                <div className="bg-red-50 text-red-700 rounded-xl p-2 font-semibold text-center">❌ 1–40: Contaminated</div>
                <div className="bg-yellow-50 text-yellow-700 rounded-xl p-2 font-semibold text-center">⚠️ 41–79: Borderline</div>
                <div className="bg-green-50 text-green-700 rounded-xl p-2 font-semibold text-center">✅ 80–100: Pure</div>
              </div>
