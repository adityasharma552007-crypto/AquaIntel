...
const ADULTERANT_LABELS: Record<string, { label: string; safeLimit: number; unit: string }> = {
  ...
  lead:      { label: 'Lead',      safeLimit: 0.5, unit: '%' },
  arsenic:    { label: 'Arsenic',    safeLimit: 0.5, unit: '%' },
  bacteria: { label: 'Bacteria', safeLimit: 0.3, unit: '%' },
}
...
export function IoTResultsDisplay() {
  ...
  const handleSave = useCallback(async (result: TestResult) => {
    ...
    try {
      ...
      // Insert scan row
      const { data: scan, error: scanErr } = await supabase.from('water_data').insert({
        user_id:       user.id,
        ...
        recommendation: tier === 'safe'
          ? 'Water appears pure. Safe to consume.'
          : tier === 'warning'
      ...
      <AnimatePresence>
        {!showAI ? (
          ...
          <ExplainWithAI
            ...
            resultTier={tier}
            recommendation={
              tier === 'safe' ? 'Water appears pure. Safe to consume.'
