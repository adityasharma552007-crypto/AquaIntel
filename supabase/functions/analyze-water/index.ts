...
function getRecommendation(tier: string, adulterants: AdulterantResult[]) {
  ...
  switch (tier) {
    case "safe":
      ...
      return warn
        ? `Water is safe but ${warn.name.toLowerCase()} is slightly below standard. Consider switching vendors if this repeats.`
        : "This water is safe to consume. No significant adulterants detected."
    case "warning":
      return "This water has minor quality issues. Not immediately dangerous but below FSSAI standards. You are not getting full nutritional value."
    case "danger":
      return "This water is adulterated above safe limits. Do not give to children. Report this vendor."
    case "hazard":
